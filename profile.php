...
<style>.pointerr{ cursor: pointer;}</style>
                ...
                <div class="row user-box">
                    ...
                    <div class="col-xs-3 ">
                      <div class="border">
                        ...
                        <img src="<?php echo $rows['profile_path'];?>" class="img-responsive" style="max-width: 182px!important; max-height: 253px;margin: auto;">
                       <?php } else {?>
...
<div><?php if($_SESSION['user_id'] == $uid ) { ?> 
    ...
    <div class="border aviator">
                          <h4>Resume</h4>
                          <a class="pointerr" id="resumeId" usr="<?php echo $uid;?>">View</a> | 

                          <a href="#" class="pointerr" id="resumeIdAdd" usr="<?php echo $uid;?>" >Add</a>
    </div>
...
<?php if($_SESSION['user_id'] == $uid ) { ?> 
    <a class="edu" href="#" id="messagesdIdd" usr="<?php echo $uid;?>" style="border:1px solid #000; cursor: pointer; padding:5px; margin-left:5px; font-weight:bold; margin-top:5px; float:left"> Messages</a>
            ...
            <div class="">
                ...
                <script type="text/javascript">
                   $('#messagesdIdd').click(function(e){
                    ...
                    $('#resumeId').click(function(e){
                    e.preventDefault();
                    var usr=$("#resumeId").attr("usr");
                        $.ajax({
                            type:"POST",
                            url:"resume_pop.php",
                            data:{"usr":usr},    	
                    ...
                    $('#resumeIdAdd').click(function(e){
                    e.preventDefault();
                    var usr=$("#resumeIdAdd").attr("usr");
                        $.ajax({
                            type:"POST",
                            url:"resume_pop_add.php",
                            data:{"usr":usr},    	
                                success:function(result){
                                    //alert('28-s');      
                                $('.messagesd_resume_add').html(result); 
                                $('.messagesd_resume_add .allmessage').css('display','block');
                                $('.profilepop1 .profile .sendmsgpop .frdsprofile').hide();
                                    $('.sendmsgpop').css('display','none'); 
