                    ...
                    <tr>
                       ...
                       <!--     <a class="btn btn-sm btn-primary" href="<?php // base_url().'login-history/'.$record->userId; ?>" title="Login history"><i class="fa fa-history"></i></a> | -->
                            ...
                            <?php if($record->is_active == 1) { ?>
                              <a class="btn btn-sm btn-danger deleteUser deactiveUser" href="#" data-userid="<?php echo $record->userId; ?>" title="Deactivate"><i class="fa fa-trash"></i></a> <?php } else{ ?>
                            <a class="btn btn-sm btn-success activeUser" href="<?php echo base_url().'activateuser/'.$record->userId; ?>" data-userid="<?php echo $record->userId; ?>" title="Activate"><i class="fa fa-user-plus"></i></a> <?php }?>
...
<script type="text/javascript">
  ...
  jQuery(document).on("click", ".deactiveUser", function(){
		var userId = $(this).data("userid"),
			hitURL = baseURL + "deactivateuser/"+userId,
			currentRow = $(this);
		...
		if(confirmation)
		{
			...
			data : { userId : userId } 
				...
				console.log(data);
				if(data.status = true) { alert("User successfully Deactivated"); }
