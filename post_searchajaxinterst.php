...
if(isset($_REQUEST['skillList']) || isset($_REQUEST['percentage']) || isset($_REQUEST['religion']) || isset($_REQUEST['searchcoworker']) || isset($_REQUEST['age_limit']))
...
//if(isset($_REQUEST['percentage']) || isset($_REQUEST['age_limit']))
{
	  ...
	  if(isset($_REQUEST['skillList']))
  	{
	  	$skillList = $_REQUEST['skillList'];
...
#echo "<pre>";print_r($all);die;
	...
	if(isset($_REQUEST['skill']) && !empty($_REQUEST['skill'])){		
		 $querySkill = "select user_id from tbl_user_skill where skill_id=".$_REQUEST['skill'];
		$resultSkill = mysqli_query($con, $querySkill);
		$iNumRowsSkill = mysqli_num_rows($resultSkill);
		while ($dataSkill = mysqli_fetch_assoc($resultSkill)) {
				$usersdetails[] = $dataSkill;
				$userIdss[] = $dataSkill['user_id'];
		}
		$mainIds = $userIdss;
	}
